...
class Collector(object):
    ...
    def __init__(self, agentConfig, emitters, systemStats):
        log.debug("BEGIN collector init")
        ...
        # Unix System Checks
        log.debug("BEGIN set _unix_system_checks")
    ...
    def run(self, checksd=None, start_event=True):
        ...
        log.debug("BEGIN _build_payload")
        payload = self._build_payload(start_event=start_event)
        log.debug("END _build_payload")
        ...
        if self.os == 'windows':
            log.debug("START Windows checks")
            # Win32 system checks
            try:
            ...
            log.debug("START UNIX checks")
            # Unix system checks
            sys_checks = self._unix_system_checks

            log.debug("START disk checks")
            ...
            log.debug("START load checks")
            load = sys_checks['load'].check(self.agentConfig)
            payload.update(load)
            
            log.debug("START memory checks")
